  ...
  process_data = function() {
    ...
    (function(__iced_k) {
      ...
      for (i = _j = 0, _ref = loaded_data.length - 1; 0 <= _ref ? _j <= _ref : _j >= _ref; i = 0 <= _ref ? ++_j : --_j) {
        rec = loaded_data[i];
        slug = rec.slug;
        if (cc[slug] != null) loaded_data[i].base.fbcomments = cc[slug];
      }
      run_templates("item", {
        items: loaded_data
      }, "#items");
