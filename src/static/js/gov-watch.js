...
(function() {
  var BOOK, SEARCHTERM, SLUG, all_books, all_subjects, all_tags, data_callback, do_search, generate_hash, generate_url, iced, initialized, load_data, loaded_data, onhashchange, process_data, run_templates, search_term, select_item, selected_book, selected_slug, set_title, setup_searchbox, setup_summary, setup_timeline, show_watermark, skip_overview, slugify, status_filter, unslugify, update_history, wm_shown, __iced_k_noop,
    __slice = [].slice;
  ...
  setup_timeline = function() {
    return $(".item").each(function() {
      var after_today, available_height, conflict, conflict_status, el, fixed_at, gov_status, has_unknowns, height, i, implementation_status, is_good_status, item_size, last_percent, last_update, last_update_at, late, line, margins, max_numeric_date, min_numeric_date, pad, point, remove_line, stamp, stamp_class, status, status_to_hebrew, status_to_stamp_class, timeline_items, today, today_at, today_date, top, _i, _j, _ref, _ref2, _ref3, _ref4;
      pad = function(n) {
        if (n < 10) {
          return '0' + n;
        } else {
      ...
      for (i = _i = _ref = timeline_items.size() - 1; _ref <= 0 ? _i <= 0 : _i >= 0; i = _ref <= 0 ? ++_i : --_i) {
        ...
        if (point.hasClass("today")) today_at = i;
