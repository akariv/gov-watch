...
(function() {
  var BOOK, SEARCHTERM, SLUG, all_books, all_subjects, all_tags, data_callback, do_search, generate_hash, generate_url, iced, initialized, load_data, loaded_data, onhashchange, process_data, run_templates, search_term, select_item, selected_book, selected_slug, set_fb_title, setup_searchbox, setup_summary, setup_timeline, show_watermark, skip_overview, status_filter, update_history, wm_shown, __iced_k,
    __slice = [].slice;
  ...
  generate_hash = function(selected_book, search_term, slug) {
    if (slug) {
      return "!z=" + BOOK + ":" + selected_book + "|" + SLUG + ":" + slug;
    } else {
  ...
  setup_timeline = function() {
    return $(".item").each(function() {
      var after_today, available_height, conflict, el, gov_status, has_unknowns, height, i, implementation_status, is_good_status, item_size, last_percent, line, margins, max_numeric_date, min_numeric_date, pad, point, status, status_to_hebrew, timeline_items, today, top, _i, _ref, _ref2, _ref3;
      pad = function(n) {
        if (n < 10) {
          return '0' + n;
        } else {
  ...
  setup_summary = function() {
    var data, fixed, in_progress, irrelevant, news, stuck, total, workaround;
